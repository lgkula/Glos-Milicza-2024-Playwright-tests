...
test.describe('Smoke tests', () => {
    ...
    test('Visibility elements on main page test', async ({page}) => {
        ...
        await test.step('Check pinned article', async () => {
            ...
            await expect(mainPage.pinedArticleHeaderLocator,'Check pined article header').not.toHaveText("");
            await expect(mainPage.pinedArticleSubHeaderLocator,'Check pined article subtitle').not.toHaveText("");

            pinedArticleHeader = await mainPage.pinedArticleHeaderLocator.innerText();
            pinedArticleSubHeader = await mainPage.pinedArticleSubHeaderLocator.innerText();
        });
        ...
        await test.step('Check first article thumbnail data', async () => {
            firstThumbnailArticleHeader = await mainPage.articleTitlesThumbnailsLocator.nth(0).innerText();
            expect(
        ...
        await test.step('Display pined article', async () => {
            ...
            await expect(articlePage.articleHeaderLocator,'Check that article header is correct').toHaveText(pinedArticleHeader);
            await expect(
    ...
    test('Open directly news page', async ({ page }) => {
        ...
        const firstArticleTitle = await newsPage.articleTitlesThumbnailsLocator.nth(0).innerText();
    ...
    test('Open directly pined article', async ({ page }) => {
        const articlePage = new ArticlePage(page);
        const articleHeader: string = 'Niemal miliard złotych na pożyczki dla dolnośląskich przedsiębiorców'
